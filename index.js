...
app.get('/collection/laptops', async (req, res) => {
  try {
    ...
    const availableType = await Products.distinct('features.type', { collectionType : 'laptops' })
    ...
    if(products && products.length > 0){
      res.status(200).json({products, filters: {brand: availableBrands, ram: availableRams, type: availableType, ssd: availableSsds, processorXbrand: availableProcessorBrands, processorXgeneration: availableProcessorGeneration, processorXname: availableProcessorNames}})
    } else {
      res.status(404).json({message: "No products found matching the criteria.", filters: {brand: availableBrands, ram: availableRams, type: availableType, ssd: availableSsds, processorXbrand: availableProcessorBrands, processorXgeneration: availableProcessorGeneration, processorXname: availableProcessorNames}})
    }
  } catch (err) {
